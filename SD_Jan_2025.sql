/* Supreme Dumplings Revenue Analysis January 2025 */

CREATE DATABASE IF NOT EXISTS sd_jan_2025;
USE sd_jan_2025;

-- Create Staging Tables (prevent loading errors)

CREATE TABLE stg_orders_jan (
	order_no VARCHAR(50),
...
SELECT * FROM stg_orders_jan;
SELECT * FROM stg_menu_jan;
SELECT * FROM stg_labor_jan;

-- Build Clean Tables

CREATE TABLE clean_orders_jan AS
SELECT
...
SELECT * FROM clean_orders_jan;

DROP TABLE IF EXISTS clean_menu_jan;
CREATE TABLE clean_menu_jan AS 
SELECT 
	menu_item,
    menu_group,
    menu,
...
SELECT * FROM clean_menu_jan;

DROP TABLE IF EXISTS clean_labor_jan;
CREATE TABLE clean_labor_jan AS
SELECT
...
SELECT * FROM clean_labor_jan;

-- Daily KPIs

DROP VIEW IF EXISTS daily_kpis_jan;
CREATE VIEW daily_kpis_jan AS
SELECT
	order_date,
    weekday,
    COUNT(*) AS orders,
    ...
    END AS discount_pct
FROM clean_orders_jan
GROUP BY order_date, weekday
ORDER BY order_date;

SELECT * FROM daily_kpis_jan;

-- Server KPIs

CREATE TABLE server_summary_jan AS
SELECT
  server,
  COUNT(*) AS orders,
  SUM(net_sales) AS revenue,
...
SELECT * 
FROM server_summary_jan
ORDER BY tips_pct DESC;

/* Observations/Insights:
- Servers who handle more table drive the majority of total revenue.
Operational efficiency and table allocation significantly impact sales performance. 
	- Thomas Cao: $26,633 (339 orders)
	- Felix Phan: $23,569 (356 orders)
	- Chris Chau: $21,744 (309 orders)
...
- Top Overall Performers
	- Anthony Le: Highest AOV and strong tip percentage,
    excellent at maximizing revenue per customer.
	- Thomas Cao: Highest revenue, highest tip percentage,
    and high AOV — strongest overall performer.
*/
...
/* Observations/Insights:
- The results show that the strongest revenue days are not limited to Saturday,
but also include Friday and Sunday in January.
*/
...
/* Observations/Insights:
- Saturday delivers the highest revenue and spending per customer,
making it the most valuable operational day.
- Weekdays (Monday–Thursday) generate significantly lower revenue due to
reduced traffic and lower spending.
- Sunday demonstrates strong revenue efficiency with high spending per
visit despite lower order volume.
*/
...
- Alcohol sales account for 50% of total drink revenue, indicating that alcoholic beverages represent a major
